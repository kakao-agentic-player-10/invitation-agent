...
from invitation_agent.services.calendar_token_store import get_stored_access_token
...
def get_calendar_token() -> str:
    ...
    if token := get_stored_access_token():
        return token

    raise CalendarAuthError(
        "캘린더 access token 이 없습니다. PlayMCP가 Authorization 헤더나 token 헤더로 "
        "토큰을 전달하지 않았고, 서버에 저장된 OAuth 토큰도 없습니다. "
        "PlayMCP에서 이 MCP의 인증하기를 다시 진행해 주세요. "
        f"현재 요청 헤더: {_safe_header_names(headers)}"
    )
